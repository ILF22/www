...
if (isset($_FILES["file"])){		
	...
	$carpeta = "imagenes/";
//Validaciones con mensajes para la subida de imagenes por tamaña, tipo y formato.
   if ($tipo != 'image/jpg' && $tipo != 'image/jpeg' && $tipo != 'image/png' && $tipo != 'image/gif'){
			echo "<script>alert('Error, el archivo no es una imagen');location.href = 'paginausuarios.php'</script></script>"; 
    }else if ($size > 5000*3500){
			echo "<script>alert('Error, el tamaño máximo permitido es un 5MB');location.href = 'paginausuarios.php'</script></script>";
    }else if ($width > 5000 || $height > 3500){
